...
class MgsHandlerBase {
    ...
    _SYS_LIST_USERS(args) {
        ...
        for (let nick in this._clients) {
            msg.data.push({
                ...
                'client': this._clients[nick].profile.id,
    ...
    _GAME_CREATE(args) {
        ...
        if (!args && !args.room && args.room.length === 0) {
            let msg = new this.Msg();
            msg.status = 'error';
            msg.msg = 'Room arg no found';
            this._conn.sendUTF(JSON.stringify(msg));
        } else if (this._game.rooms.length > 0 && this._game.rooms.indexOf(args.room) !== -1) {
            let msg = new this.Msg();
            msg.status = 'error';
    ...
    _GAME_JOIN(args) {
        ...
        if (!args && !args.room && args.room.length === 0) {
            ...
            msg.msg = 'Room arg no found';
            this._conn.sendUTF(JSON.stringify(msg));
        } else if (this._game.rooms.indexOf(args.room) === -1) {
            let msg = new this.Msg();
            msg.status = 'error';
            ...
            this._conn.profile.position = this._game[args.room].clients.length;
            this._conn.profile.room = args.room;
            this._game.rooms[args.room].turns.push(this._conn.profile.nick);
            this._game.rooms[args.room].clients[this._conn.profile.nick] = {
                score: 0
